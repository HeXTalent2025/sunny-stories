...
export default async function handler(req, res) {
  ...
  const { stories, sessionId, giftRecipient, giftOccasion } = req.body;
  ...
  const OCCASION_LABELS = {
    ...
    'just-because': 'Just Because', 'new-baby': 'New Baby', 'thank-you': 'Thank You',
  };
  const giftLink = [
    `${appUrl}/gift?token=${token}`,
    giftRecipient ? `&name=${encodeURIComponent(giftRecipient)}` : '',
    giftOccasion  ? `&occasion=${encodeURIComponent(giftOccasion)}` : '',
  ].join('');
  const occasionLabel = OCCASION_LABELS[giftOccasion] || 'gift';
  const giftBtnText = giftRecipient
    ? `🎁 Open ${giftRecipient}'s ${occasionLabel} stories`
    : `🎁 Open your ${occasionLabel} stories`;
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:28px 0 0;">
          <tr>
            <td bgcolor="#fff5f3" style="border-radius:14px;padding:20px 24px;border:1px solid #f0c8bc;">
              <div style="font-size:11px;font-weight:700;color:#d86e59;letter-spacing:1px;margin-bottom:10px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">🎁 ${giftRecipient ? `GIFT LINK FOR ${giftRecipient.toUpperCase()}` : `YOUR ${occasionLabel.toUpperCase()} GIFT LINK`}</div>
              <p style="margin:0 0 16px;font-size:14px;color:#4a6070;line-height:1.6;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
                Share this link${giftRecipient ? ` with ${giftRecipient}` : ''} — they'll see a beautiful${giftOccasion ? ` ${occasionLabel}` : ''} gift page with ${stories.length === 1 ? 'their story' : `all ${stories.length} stories`} waiting inside.
              </p>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td bgcolor="#d86e59" style="border-radius:50px;padding:14px 32px;">
                          <a href="${giftLink}" style="color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;display:block;white-space:nowrap;">${giftBtnText}</a>
                        </td>
                      </tr>
                    </table>
                  </td>
